...
function loop() {
  setInterval(function() {
    ...
    if (gameOver == false) {
      ...
      draw();
      globalvar();
      makeCanvasCoverFullScreen(800, 400);

      if (gameOver == false) {
        document.getElementById("timer_title").innerHTML = "Timer: " + seconds;
      ...
      $("#high_score_text").html("High Score: " + best);
    }
  }, 10);
}
...
function update() {
  ...
  if (seconds > 89) {
    $("#speech").html("You can no longer keep going further, you now have a tiny area to move around.");
    mover = true;
  }
  else if (seconds > 59) {
    $("#speech").html("Your spaceship's blasters are jammed. You can no longer shoot.");
  }
  else if (seconds > 29) {
    $("#speech").html("More UfOs have apeared, and they brought a laser machine.");
  }
  else if (seconds > 9) {
    $("#speech").html("A UFO has apeared out of no where! Use your spaceship's gun to destroy the UFO!");
  }
  else {
    $("#speech").html("Asteroids are coming in! Try not to get hit by them.");
  }
